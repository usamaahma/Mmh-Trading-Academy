    ...
    return (
        <div className="min-h-screen bg-[#010409] text-white p-6 md:p-20">
            ...
            <div className="mb-16 mt-15">
                ...
                <div className="flex gap-4 mt-8 border-b border-white/5 pb-4 overflow-x-auto no-scrollbar">
                    {["ALL", "FOREX", "STOCKS", "CRYPTO"].map((cat) => (
                        <button
                            key={cat}
                            onClick={() => window.history.pushState(null, "", `?category=${cat}`)}
                            className={`text-[10px] font-bold tracking-[0.2em] uppercase transition-all ${activeCategory === cat ? "text-cyan-500" : "text-slate-500 hover:text-white"
                                }`}
                        >
                            {cat}
                        </button>
            ...
            <div className="max-w-4xl space-y-12">
                {loading ? (
                    <p className="text-cyan-500 font-mono text-xs animate-pulse">SYSTEM_LOADING...</p>
                ) : (
                    analysisData.map((item) => (
                        <div key={item._id} className="group border-b border-white/5 pb-12 flex flex-col md:flex-row gap-8">
                            ...
                            >
                                <img src={item.image} className="w-full h-full object-cover opacity-80 group-hover:opacity-100 transition-opacity" alt="Chart" />
...
export default function AnalysisPage() {
    return (
        <Suspense fallback={<div className="p-20 text-white font-mono text-xs">INITIALIZING...</div>}>
            <AnalysisContent />
        </Suspense>
    );
}