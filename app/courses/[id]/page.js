...
export default function CourseDetailPage() {
  ...
  return (
    <div className="min-h-screen bg-[#010409] text-white pt-16 md:pt-24 pb-10 md:pb-20 px-4 md:px-6 font-sans overflow-x-hidden w-full">
      <div className="max-w-4xl mx-auto w-full">
        ...
        <header className="mb-12 md:mb-16">
          <h1 className="text-3xl md:text-6xl font-black italic uppercase tracking-tighter mb-4 md:mb-6 text-white leading-[1] md:leading-[0.9] break-words">
            {course.courseName}
          </h1>
          <div
            className="text-slate-400 text-[10px] md:text-xs font-bold uppercase tracking-[0.15em] border-l-2 border-cyan-500 pl-4 md:pl-6 py-1 max-w-2xl leading-relaxed break-words"
            dangerouslySetInnerHTML={{ __html: course.shortDescription }}
          />
        </header>

        {/* 1. SYLLABUS OVERVIEW (Responsive Fix) */}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4 w-full">
            {course.sections?.map((section, idx) => (
              <button
                ...
                className="bg-[#0D1117] border border-white/5 p-5 md:p-6 rounded-xl md:rounded-2xl hover:border-cyan-500/40 transition-all text-left group shadow-xl flex flex-col h-full active:scale-[0.98] w-full"
              >
                <span className="text-cyan-500 font-black text-[8px] md:text-[9px] block mb-2 tracking-widest">
                  MOD-0{idx + 1}
                </span>
                <span className="text-[12px] md:text-xs font-black text-white uppercase italic tracking-tight mb-4 group-hover:text-cyan-400 transition-colors line-clamp-2 break-words">
                  {section.heading}
                </span>
        ...
        <div className="space-y-20 md:space-y-32 mb-20 md:mb-40 w-full">
          {course.sections?.map((section, idx) => (
            <section
              key={idx}
              id={`section-${idx}`}
              className="scroll-mt-24 group relative w-full"
            >
              ...
              <div className="mb-6 md:mb-8">
                ...
                <h2 className="text-2xl md:text-3xl font-black italic uppercase text-white tracking-tighter leading-tight break-words">
                  {section.heading}
                </h2>
              </div>
              ...
              {/* CONTENT AREA */}
              <div
                className="text-white text-[14px] md:text-[15px] leading-relaxed mb-8 md:mb-10 break-words w-full
              ...
              <div className="flex flex-col sm:flex-row gap-3 md:gap-4 w-full">
                {section.youtubeLink && (
                  <a
                    ...
                    className="flex items-center gap-4 bg-[#0D1117] border border-white/10 px-5 py-4 rounded-xl hover:bg-red-500/10 hover:border-red-500/30 transition-all group/btn w-full sm:w-auto"
                  >
                ...
                {section.megaLink && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-4 bg-[#0D1117] border border-white/10 px-5 py-4 rounded-xl hover:bg-cyan-500/10 hover:border-cyan-500/30 transition-all group/btn w-full sm:w-auto"
                  >
        ...
        {course.faqs && course.faqs.length > 0 && (
          <div className="border-t border-white/10 pt-16 md:pt-20 w-full">
            ...
            <div className="grid gap-4 md:gap-6 w-full">
              {course.faqs.map((faq, i) => (
                <div
                  key={i}
                  className="group bg-[#0D1117]/40 p-6 md:p-8 rounded-2xl md:rounded-[2rem] border border-white/5 hover:border-cyan-500/20 transition-all shadow-lg w-full"
                >
                  <h4 className="font-black text-[11px] md:text-xs text-white mb-3 uppercase tracking-tight italic flex gap-2 break-words">
                    <span className="text-cyan-500 not-italic">Q.</span>{" "}
                    {faq.question}
                  </h4>
                  <div
                    className="text-white text-[10px] md:text-[11px] leading-relaxed font-medium pl-4 border-l border-white/10 group-hover:border-cyan-500 transition-colors break-words
