...
export default function AdminSidebar({ activeTab, setActiveTab }) {
  const menuItems = [
    ...
    { id: "brokers", label: "Add Brokers", icon: <BookOpen size={18} /> },
  ...
  return (
    <aside className="w-64 bg-[#0D1117] border-r border-white/5 flex flex-col h-screen sticky top-0">
      <div className="p-6 border-b border-white/5">
        <h2 className="text-cyan-500 font-black tracking-tighter text-xl italic">ADMIN TERMINAL</h2>
        <p className="text-[8px] text-slate-500 uppercase tracking-widest mt-1">Control Center v3.0</p>
      </div>
      ...
      <nav className="flex-1 p-4 space-y-2">
        {menuItems.map((item) => (
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-xs font-bold uppercase tracking-widest transition-all ${activeTab === item.id
